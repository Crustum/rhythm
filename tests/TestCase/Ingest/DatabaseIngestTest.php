...
class DatabaseIngestTest extends TestCase
{
    ...
    {
        FrozenTime::setTestNow('2000-01-01 00:00:04');
        $this->rhythm->set('type', 'foo', 'value');
        FrozenTime::setTestNow('2000-01-01 00:00:05');
        $this->rhythm->set('type', 'bar', 'value');
        FrozenTime::setTestNow('2000-01-01 00:00:06');
        $this->rhythm->set('type', 'baz', 'value');

        $this->rhythm->ingest();

        $this->rhythm->stopRecording();
        FrozenTime::setTestNow('2000-01-08 00:00:05');
        $this->rhythm->trim();
    ...
    {
        FrozenTime::setTestNow('2000-01-01 00:00:59'); // Bucket: 2000-01-01 00:00:00
        $this->rhythm->record('foo', 'xxxx', 1);
        $this->rhythm->ingest();
        $this->assertEquals(1, $this->Aggregates->find()->where(['period' => 60])->count());

        FrozenTime::setTestNow('2000-01-01 00:01:00'); // Bucket: 2000-01-01 00:01:00
        $this->rhythm->record('foo', 'xxxx', 1);
        $this->rhythm->ingest();
        $this->assertEquals(2, $this->Aggregates->find()->where(['period' => 60])->count());

        $this->rhythm->stopRecording();
        FrozenTime::setTestNow('2000-01-01 00:59:59'); // 1 second before the oldest bucket become irrelevant.
        $this->rhythm->trim();
        $this->assertEquals(2, $this->Aggregates->find()->where(['period' => 60])->count());

        FrozenTime::setTestNow('2000-01-01 01:00:00'); // The second the oldest bucket become irrelevant.
        $this->rhythm->trim();
        $this->assertEquals(1, $this->Aggregates->find()->where(['period' => 60])->count());
    }
    ...
    {
        FrozenTime::setTestNow('2000-01-01 00:05:59'); // Bucket: 2000-01-01 00:00:00
        $this->rhythm->record('foo', 'xxxx', 1);
        $this->rhythm->ingest();
        $this->assertEquals(1, $this->Aggregates->find()->where(['period' => 360])->count());

        FrozenTime::setTestNow('2000-01-01 00:06:00'); // Bucket: 2000-01-01 00:06:00
        $this->rhythm->record('foo', 'xxxx', 1);
        $this->rhythm->ingest();
        $this->assertEquals(2, $this->Aggregates->find()->where(['period' => 360])->count());

        $this->rhythm->stopRecording();
        FrozenTime::setTestNow('2000-01-01 05:59:59'); // 1 second before the oldest bucket become irrelevant.
        $this->rhythm->trim();
        $this->assertEquals(2, $this->Aggregates->find()->where(['period' => 360])->count());

        FrozenTime::setTestNow('2000-01-01 06:00:00'); // The second the oldest bucket become irrelevant.
        $this->rhythm->trim();
        $this->assertEquals(1, $this->Aggregates->find()->where(['period' => 360])->count());
    }
    ...
    {
        FrozenTime::setTestNow('2000-01-01 00:23:59'); // Bucket: 2000-01-01 00:00:00
        $this->rhythm->record('foo', 'xxxx', 1);
        $this->rhythm->ingest();
        $this->assertEquals(1, $this->Aggregates->find()->where(['period' => 1440])->count());

        FrozenTime::setTestNow('2000-01-01 00:24:00'); // Bucket: 2000-01-01 00:24:00
        $this->rhythm->record('foo', 'xxxx', 1);
        $this->rhythm->ingest();
        $this->assertEquals(2, $this->Aggregates->find()->where(['period' => 1440])->count());

        $this->rhythm->stopRecording();
        FrozenTime::setTestNow('2000-01-01 23:35:59'); // 1 second before the oldest bucket become irrelevant.
        $this->rhythm->trim();
        $this->assertEquals(2, $this->Aggregates->find()->where(['period' => 1440])->count());

        FrozenTime::setTestNow('2000-01-02 00:00:00'); // The second the oldest bucket become irrelevant.
        $this->rhythm->trim();
        $this->assertEquals(1, $this->Aggregates->find()->where(['period' => 1440])->count());
    }
    ...
    public function testTrimsAggregatesToTheConfiguredStorageDurationWhenConfiguredTrimIsShorterThanTheBucketPeriodDuration(): void
    {
        ...
        FrozenTime::setTestNow('2000-01-01 00:00:00'); // Bucket: 2000-01-01 00:00:00
        $this->rhythm->record('foo', 'xxxx', 1);
        $this->rhythm->ingest();
        $this->assertEquals(1, $this->Aggregates->find()->where(['period' => 1440])->count());

        FrozenTime::setTestNow('2000-01-01 00:22:59');
        $this->rhythm->trim();
        $this->assertEquals(1, $this->Aggregates->find()->where(['period' => 1440])->count());

        FrozenTime::setTestNow('2000-01-01 00:23:00');
        $this->rhythm->trim();
    ...
    {
        FrozenTime::setTestNow('2000-01-01 02:23:59'); // Bucket: 1999-12-31 23:36:00
        $this->rhythm->record('foo', 'xxxx', 1);
        $this->rhythm->ingest();
        $this->assertEquals(1, $this->Aggregates->find()->where(['period' => 10080])->count());

        FrozenTime::setTestNow('2000-01-01 02:24:00'); // Bucket: 2000-01-01 02:24:00
        $this->rhythm->record('foo', 'xxxx', 1);
        $this->rhythm->ingest();
        $this->assertEquals(2, $this->Aggregates->find()->where(['period' => 10080])->count());

        $this->rhythm->stopRecording();
        FrozenTime::setTestNow('2000-01-07 23:35:59'); // 1 second before the oldest bucket become irrelevant.
        $this->rhythm->trim();
        $this->assertEquals(2, $this->Aggregates->find()->where(['period' => 10080])->count());

        FrozenTime::setTestNow('2000-01-07 23:36:00'); // The second the oldest bucket become irrelevant.
        $this->rhythm->trim();
        $this->assertEquals(1, $this->Aggregates->find()->where(['period' => 10080])->count());
    }
    ...
    public function testCanConfigureDaysOfDataToKeepWhenTrimming(): void
    {
        ...
        $this->rhythm->stopRecording();
        FrozenTime::setTestNow('2000-01-03 00:00:05');
        $this->rhythm->trim();
